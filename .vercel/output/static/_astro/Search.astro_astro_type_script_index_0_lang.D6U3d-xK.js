  ...
  E = function (l, i, c) {
    ...
    if (i && i.length > 0) {
      ...
      const s = document.querySelector("meta[property=csp-nonce]"),
        n = s?.nonce || s?.getAttribute("nonce");
      d = Promise.all(
        i.map((t) => {
          if (((t = y(t)), t in g)) return;
          g[t] = !0;
          const r = t.endsWith(".css"),
            f = r ? '[rel="stylesheet"]' : "";
          if (document.querySelector(`link[href="${t}"]${f}`)) return;
            ...
            return new Promise((u, e) => {
              o.addEventListener("load", u),
                o.addEventListener("error", () =>
                  e(new Error(`Unable to preload CSS for ${t}`)),
                );
            });
        }),
      );
...
class S extends HTMLElement {
  constructor() {
    super();
    const l = this.querySelector("button[data-open-modal]"),
      i = this.querySelector("button[data-close-modal]"),
      c = this.querySelector("dialog"),
      d = this.querySelector(".dialog-frame"),
      s = (e) => {
        ("href" in (e.target || {}) ||
          (document.body.contains(e.target) && !d.contains(e.target))) &&
          t();
      },
      n = (e) => {
        c.showModal(),
    ...
    l.addEventListener("click", n),
      ...
      window.addEventListener("keydown", (e) => {
        (e.metaKey === !0 || e.ctrlKey === !0) &&
          e.key === "k" &&
          (c.open ? t() : n(), e.preventDefault());
      });
    let r = {};
    try {
      r = JSON.parse(this.dataset.translations || "{}");
    } catch {}
    const u =
    ...
    window.addEventListener("DOMContentLoaded", () => {
      (window.requestIdleCallback || ((h) => setTimeout(h, 1)))(async () => {
        const { PagefindUI: h } = await E(async () => {
          const { PagefindUI: a } = await import("./ui-core.DlYJqfej.js");
          return { PagefindUI: a };
        }, []);
        new h({
